...
import { getListAllVideo } from '../../utilities/FirebaseDatabase';
...
function AllVideoScreen({navigation}) {
  ...
  const [listVideoDisplay, setListVideoDisplay] = useState([]);
  ...
  useEffect(() => {
    const list = listVideo?.filter((item)=>{
      return item?.name?.toLowerCase()?.includes(textInput?.toLowerCase())
    })
    setListVideoDisplay(list)
  }, [textInput, listVideo]);

  const getData = async () => {
    try {
      const res = await getListAllVideo();
      if (!res.val()) throw 'FAIL TO GET VIDEO';
      const list = convertObjectToArrayWithoutKey(res.val())
      setListVideo(list);
    } catch (e) {
      console.log(e);
    }
